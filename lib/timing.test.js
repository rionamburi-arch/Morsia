...
test('gaps: non-positive or missing effWpm falls back to standard spacing', () => {
  assert.deepEqual(gaps(20, 0), gaps(20));
  assert.deepEqual(gaps(20, -5), gaps(20));
  assert.deepEqual(gaps(20, undefined), gaps(20));
});

test('unitMs and toSegments guard non-positive wpm', () => {
  assert.throws(() => unitMs(0), RangeError);
  assert.throws(() => unitMs(NaN), RangeError);
  assert.throws(() => toSegments('E', {}), RangeError);
});
...
test('toSegments: charIndex increments per known char, wordIndex per word with content', () => {
  ...
  const wordGap = toSegments('E E', { wpm: 20 }).find((s) => s.kind === 'word');
  assert.equal(wordGap.charIndex, -1);
  assert.equal(wordGap.wordIndex, -1);
  const charGap = toSegments('EE', { wpm: 20 }).find((s) => s.kind === 'char');
  assert.equal(charGap.wordIndex, 0);
});
