...
/**
 * The dichotomic search tree: from the root, a dit takes the left branch and a
 * dah the right. Letters only, depth 4 — digits would make it depth 5 and 32
 * leaves wide, which stops being readable.
 */
export function treeNodes(maxDepth = 4) {
  const byPattern = new Map(
    Object.entries(TABLE).filter(([c, p]) => isLetter(c) && p.length <= maxDepth).map(([c, p]) => [p, c]),
  );
  const build = (pattern, depth) => {
    const node = { pattern, char: byPattern.get(pattern) ?? null };
    if (depth < maxDepth) {
      node.dit = build(`${pattern}.`, depth + 1);
      node.dah = build(`${pattern}-`, depth + 1);
    }
    return node;
  };
  return build('', 0);
