...
export function unitMs(wpm) {
  if (!(wpm > 0)) throw new RangeError('wpm must be a positive number');
  return 1200 / wpm;
}
...
export function gaps(wpm, effWpm = wpm) {
  const u = unitMs(wpm);
  const e = effWpm > 0 ? Math.min(effWpm, wpm) : wpm;
 ...
 * dit/dah/intra carry the character they belong to; char gaps carry the current
 * wordIndex; word gaps carry charIndex: -1 and wordIndex: -1. Never two gaps
 * adjacent, no leading/trailing gap.
 */
export function toSegments(text, { wpm, effWpm = wpm } = {}) {
  ...
  for (const word of words) {
    ...
    if (wordIndex > 0) out.push({ on: false, ms: g.word, kind: 'word', char: null, charIndex: -1, wordIndex: -1 });
    chars.forEach((ch, ci) => {
      const code = TABLE[ch];
...
/**
 * A single run-together pattern ('-.-', or a prosign like '...---...') → segments.
 * Elements separated by one unit; no leading or trailing gap. Used by the chart,
 * where the thing being played is a pattern rather than text.
 */
export function patternToSegments(pattern, { wpm } = {}) {
  const u = unitMs(wpm);
  const els = [...String(pattern)].filter((c) => c === '.' || c === '-');
  const out = [];
  els.forEach((c, i) => {
    const dah = c === '-';
    out.push({ on: true, ms: dah ? 3 * u : u, kind: dah ? 'dah' : 'dit', char: null, charIndex: 0, wordIndex: 0 });
    if (i < els.length - 1) out.push({ on: false, ms: u, kind: 'intra', char: null, charIndex: 0, wordIndex: 0 });
  });
  return out;
}
