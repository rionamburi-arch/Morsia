...
test('a second play stops the first; stop is idempotent; now() reads context time', () => {
  ...
  const firstGainEvents = last.gains[0].gain.events;
...
test('keyDown anchors the ramp', () => {
  const e = make();
  e.keyDown();
  assert.deepEqual(last.gains[0].gain.events[0], ['set', 0, 10]);
  assert.deepEqual(last.gains[0].gain.events[1], ['ramp', 0.35, 10.004]);
});
