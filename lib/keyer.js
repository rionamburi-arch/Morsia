...
export function createKeyer({ wpm = 18 } = {}) {
  ...
  function adapt(dur) {
    ...
    const mid = Math.sqrt(min * max); // geometric split between dits and dahs
    const dits = history.filter((d) => d <= mid);
  ...
  function settle(t) {
    ...
    if (pattern && silence >= th.charGap) {
      out.push({ type: 'character', pattern, char: REVERSE[pattern] ?? '?' });
      pattern = '';
      awaitingWord = true;
    }
